...
- **Explicit project modes** - each project ships via `no-mistakes`, `direct-PR`, `direct-push`, or `local-only`, with an optional `+yolo` autonomy flag.
  `direct-push` runs the full no-mistakes pipeline and then pushes the validated branch to `origin`, for forges firstmate cannot open PRs on such as Bitbucket.
