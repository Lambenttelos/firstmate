...
# Each ship-spawn case below creates its project as a direct child of TMP_ROOT
# and passes it by absolute path, so FM_PROJECTS_OVERRIDE points at TMP_ROOT:
# fm-spawn's own-clone assertion requires a ship/scout project to be a direct
# child of this home's projects directory. (The --secondmate case sets its own
# override and is exempt from that check.)
...
test_spawn_preserves_orca_metadata_when_pathless_worktree_cleanup_fails() {
  ...
  out=$( PATH="$FB:$PATH" FM_ORCA_LOG="$LOG" FM_ORCA_RESPONSES="$RESP" \
    ...
    FM_PROJECTS_OVERRIDE="$TMP_ROOT" FM_SPAWN_NO_GUARD=1 \
