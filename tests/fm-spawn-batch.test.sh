...
# Config is pointed at an empty directory rather than cleared: an empty
# FM_CONFIG_OVERRIDE falls back to $FM_ROOT/config, and a task worktree carries
# the home's real gitignored config files, so clearing it let a live
# config/crew-dispatch.json reach the spawn and refuse every case in this suite
# with the dispatch-consultation backstop.
