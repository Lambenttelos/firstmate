...
config/token-prices.json  shared token-price snapshot, TRACKED (the one non-local config file); the owned per-model USD-per-Mtok anthropic price table that bin/fm-token-lib.sh costs token usage against; written ONLY by bin/fm-token-prices.sh --refresh as a straight copy of providers.anthropic out of jcode's cached models.dev feed, with a header carrying price_source, the source's cached_at, and the snapshot's written_at so every price is traceable from the file alone; never hand-edited (one-owner-per-contract; bin/fm-token-prices.sh header owns the format, design PR-T1)
...
state/               volatile runtime signals; gitignored
  ...
  <id>.telemetry     key=value, same format as <id>.meta; shared per-task producer artifact written in place by bin/fm-telemetry-lib.sh's fm_telemetry_set (one key updated without clobbering sibling keys); the watcher's quota_anomaly_scan (Visibility Gap-2) writes count_429/last_429_ts here on a pane 429/rate-limit tripwire; removed by teardown
