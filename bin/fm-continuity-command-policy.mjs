...
// fleet scripts and divides them into recovery commands (wake drain, the wake
// brief that batches that same drain with plain reads, watcher arm, and
// fail-closed teardown) versus every other bin/fm-*.sh command. Unparseable or
// opaque dynamic commands fail open so this gate can never become a blanket
// shell block.
...
// fm-wake-brief.sh belongs here for the same reason fm-wake-drain.sh does: it IS
// that drain plus read-only reads of state this gate never protected, so denying
// it would only push recovery back to the slower unbatched sequence it replaces.
const RECOVERY_SCRIPTS = new Set(["fm-wake-drain.sh", "fm-wake-brief.sh", "fm-watch-arm.sh", "fm-teardown.sh"]);
