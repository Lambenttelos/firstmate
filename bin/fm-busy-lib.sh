...
#   jcode-hook       jcode lifecycle hooks (turn_start/turn_end/session_end),
#                    configured per task by JCODE_HOOK_* environment overrides
...
# fm_busy_record_ts: the ts= epoch of a VALID, gen-matching record, or failure.
# Reuses fm_busy_record_read for validation so only a trusted, current record
# yields a timestamp; a missing, malformed, or stale record yields nothing.
# Cost consumers (the fm-send cold-cache guard) read the last-event epoch here
# rather than re-deriving the record contract a second time.
fm_busy_record_ts() {  # <state-dir> <id>
  local rec line f ts=''
  fm_busy_record_read "$1" "$2" >/dev/null || return 1
  rec=$(fm_busy_record_path "$1" "$2")
  IFS= read -r line < "$rec" 2>/dev/null || return 1
  local -a fields
  IFS=' ' read -r -a fields <<< "$line"
  for f in "${fields[@]:1}"; do
    case "$f" in ts=*) ts=${f#ts=} ;; esac
  done
