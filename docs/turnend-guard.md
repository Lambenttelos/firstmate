...
The knob is resolved by `fm_watch_absorb_tick_enabled` in `bin/fm-supervision-lib.sh` from the same sources the watcher sees, shared with the pull-based `bin/fm-guard.sh` so the two postures cannot drift.
