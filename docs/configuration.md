...
The read is claude/jcode-capable, using the same per-harness readers the secondmate monitor uses (claude and jcode each have their own; see docs/secondmate-context-handoff.md); it fails closed, so on any other harness or any unreadable count the check simply never nudges.
