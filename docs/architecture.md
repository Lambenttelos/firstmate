...
Supervision ownership follows an actually-live daemon, never the away-mode flag alone: `bin/fm-afk-daemon-lib.sh` answers that one question for the turn-end guard, the watcher's triage gate, the pull-based guard, and the session-start digest, so a home that runs away mode without a daemon (no injectable supervisor pane to escalate into) keeps arming, triaging, and repairing its own watcher exactly as in normal mode.
