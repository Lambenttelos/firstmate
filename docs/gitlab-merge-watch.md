...
`bin/fm-pr-merge.sh` addresses GitHub (by owner and repository) and Bitbucket Cloud (by workspace and repository, see `docs/bitbucket-pr.md`), but not GitLab.
It refuses a GitLab merge request URL rather than sending it to the wrong forge, so merging a GitLab merge request stays a deliberate manual step until GitLab merge parity lands separately.
