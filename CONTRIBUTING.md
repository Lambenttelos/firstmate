...
Raising every change through no-mistakes is the expected contributor workflow, so its review, test, lint, and CI steps run before the change reaches `main`.
...
- Only shared material is tracked: `AGENTS.md`, `README.md`, `CONTRIBUTING.md`, `.tasks.toml`, `.github/workflows/`, `bin/`, `.agents/skills/`, and `skills/`.
  ...
  The root `.tasks.toml` is tracked `tasks-axi` config for `data/backlog.md`; compatible `tasks-axi` is the default backend for routine backlog mutations, with the compatibility definition owned by [`docs/configuration.md`](docs/configuration.md) ("Backlog backend"), and tracking it does not make `data/` tracked.
...
When other work is live on the same machine, run heavy commands - suites, lint sweeps, builds - through `bin/fm-heavy-run.sh -- <command>`, which queues them behind a configurable ceiling and hands back the command's real output and exit status; `docs/configuration.md` owns that ceiling.
