...
No AGENTS.md reread nudge is needed at spawn or respawn because the agent reads instructions fresh on launch; only the bootstrap sweep's running-home instruction-surface advance needs that AGENTS.md re-read, and only for a busy home: the nudge goes out only when the secondmate's own home carries in-flight work (any `state/*.meta`), and an idle home is skipped with a `BOOTSTRAP_INFO:` note because it picks the new instructions up at next routed task or respawn (lazy nudge policy, owned in `bin/fm-ff-lib.sh`).
Bootstrap reports successful AGENTS.md re-read sends as `BOOTSTRAP_INFO:`, records an idle skip the same way, and only emits `NUDGE_SECONDMATES:` when that send fails and needs retry.
A separate, literal-content config reread is required whenever inherited `config/*` material changes under an already-running secondmate, and it follows the same lazy nudge policy (`secondmate_has_inflight_work` in `bin/fm-ff-lib.sh`): the file write and propagation always happen, but the live-agent pointer send is gated on in-flight work, so an idle home is skipped with a `BOOTSTRAP_INFO:` note and its published `*.pending` instruction stays as the durable record until the home is busy again or relaunches (fresh config read at launch).
...
Secondmate project lists may include remote-backed `no-mistakes`, `direct-PR`, and `direct-push` projects only.
