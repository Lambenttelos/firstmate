...
jcode uses a tracked background-notify cycle around `bin/fm-watch-arm.sh` with a mandatory jcode-specific second step: after launching the arm as a `Bash run_in_background` task, set `wake: true` on that task id with the `bg` tool, because a jcode background task defaults to `wake: false` and a `wake: true` completion is what re-drives the idle model (`docs/jcode-wake-adapter.md`). The bounded foreground checkpoint (`bin/fm-watch-checkpoint.sh`) remains its zero-dependency fallback for non-idle supervision (`docs/jcode-primary-supervision.md`).
...
Its primary watcher protocol is a tracked background-notify cycle around `bin/fm-watch-arm.sh` with a mandatory jcode-specific wake step: after launching the arm as a `Bash run_in_background` task, set `wake: true` on that task id with the `bg` tool (subscribe/delivery), because a jcode background task defaults to `notify: true, wake: false` and only a `wake: true` completion re-drives an idle model. The cross-turn verification record is `docs/jcode-wake-adapter.md`; the earlier checkpoint-only record `docs/jcode-primary-supervision.md` remains the reference for the bounded foreground checkpoint (`bin/fm-watch-checkpoint.sh`), which stays the zero-dependency fallback for non-idle supervision.
The `wake: true` set is only reachable from the model's `bg` tool: the `jcode debug` control plane is gated off and `run_in_background` exposes no wake flag at launch, so the arm is inherently two paired actions and a forgotten wake set silently leaves supervision blind.
`ScheduleWakeup` create is currently rejected (`Error: task is required for action=create`), so it is not a usable wake primitive today.
